...
原生 SwiftUI 相册整理应用。产品范围、运行数据和构建入口见 README.md；命名及图标由 project.yaml 定义。
...
- 重复副本可能共享 Cloud GUID，真实库已复现。删除解析与选择计数必须优先使用行内 local_uuid；同时验证 Cloud GUID 未变，禁止按云标识字典覆盖选择某个副本，禁止本地资产消失后换成同云标识的另一个副本。旧清单无 local_uuid 时只允许唯一云标识匹配，否则要求重建。
- “在‘照片’中打开”使用系统 Photos.sdef 的 spotlight(media item)；仅插入验证后的 UUID，不用文件名搜索或原片路径打开，避免打开错误副本或外部预览程序。
